...
<head>
  ...
  <title>Orinetation Maroc - License</title>
  <?php
...
<body>
  ...
  <?php
          $pdo = connecto();
          $req = "SELECT * FROM invit WHERE pour = ? OR pour = ? ORDER BY ID DESC Limit 20";
          $t = $pdo->prepare($req);
          $t->execute(['LP','LF']);
        ...
        <?php } ?>
          ...
          <?php
          $pdo = connecto();
          $req = "SELECT * FROM result WHERE pour = ? OR pour = ? ORDER BY ID DESC Limit 20";
      $t = $pdo->prepare($req);
      $t->execute(['LP','LF']);
          while($row = $t->fetch(PDO::FETCH_ASSOC)){
          ?>
